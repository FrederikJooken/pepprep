...
sap.ui.controller("hcm.people.profile.ZHCM_PEP_PROFILEExt.view.ProfileCustom", {
	...
	initializeView: function(){
		...
		//Get the logged on user ID, to add it to the PiwikDataSet later on...
		var sUserId = sap.ushell.Container.getService("UserInfo").getUser().getId();  //only works when deployed on ABAP stacked Front-end server		
		...
		this.getView().getModel().read("/PiwikDatasetSet", null, null, false, function(oData, oResponse) {
			...
			var piwikDataModel = new sap.ui.model.json.JSONModel(oData);
			sap.ui.getCore().setModel(piwikDataModel, "piwikDataModel");
	...
	onIncorrectDataLinkPress: function(){
